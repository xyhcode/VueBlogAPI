...
// The GiveMoneyQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type GiveMoneyQueryRuleFunc func(context.Context, *ent.GiveMoneyQuery) error

// EvalQuery return f(ctx, q).
func (f GiveMoneyQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.GiveMoneyQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.GiveMoneyQuery", q)
}

// The GiveMoneyMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type GiveMoneyMutationRuleFunc func(context.Context, *ent.GiveMoneyMutation) error

// EvalMutation calls f(ctx, m).
func (f GiveMoneyMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.GiveMoneyMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.GiveMoneyMutation", m)
}
