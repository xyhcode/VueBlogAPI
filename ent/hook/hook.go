...
// The GiveMoneyFunc type is an adapter to allow the use of ordinary
// function as GiveMoney mutator.
type GiveMoneyFunc func(context.Context, *ent.GiveMoneyMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GiveMoneyFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GiveMoneyMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GiveMoneyMutation", m)
}
